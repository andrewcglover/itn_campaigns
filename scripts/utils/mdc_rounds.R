...
orig_generate_MDC_round_matrices <- function(max_tau = 12) {
...
# Generate MDC round matrices
generate_MDC_round_matrices <- function(use_ranked_tau = FALSE,
                                        max_tau = 6) {
  ...
  MDC_rounds <- data.frame("ISO2" = net_data$ISO2,
                            ...
                            "CMC" = net_data$CMC)
  if (use_ranked_tau) {
    MDC_rounds$tau <- net_data$ranked_tau
  } else {
    MDC_rounds$tau <- net_data$mdc_tau
  }
  MDC_rounds <- MDC_rounds[which(net_data$mdc),]
  MDC_rounds$area_id <- match(MDC_rounds$area, unique_areas_included)
  max_rounds <- max(MDC_rounds$round) + 1
  ...
  return(list(MDC_matrix, MDC_tau_matrix))
}