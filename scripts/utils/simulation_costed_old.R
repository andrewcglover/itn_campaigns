...
par_net_region_sequential_costed <- function(param_list) {
  ...
  npc_new <- cost_factor * npc_new #new net cost adjustment
  
  # Calculate new costed use
  avg_use_new <- bv_use[which(abs(npc_new-bv_npc) == (min(abs(npc_new-bv_npc))))]
  avg_C_new <- avg_use_new - D_end
  C0_new <- (avg_C_new * lambda * mass_int_mn) / (1 - exp(-lambda*mass_int_mn))
  P0_new <- C0_new + D_end
  if (P0_new > 1) {
    P0_new <- 1
    C0_new <- P0_new - D_end
  ...
  # Extend values
  P0_long <- c(P0, rep(P0_end, projection_window_mn))#P0_new, projection_window_mn))
  ...
  if (no_future_nets) {
    ...
    all_output_nets[new_net_range] <- rep(tail(all_output_nets,
                                               mass_int_mn),
                                          length.out = length(new_net_range))
  }
  ...
  avg_pop_adj_tail_nets <- avg_tail_nets * npc_new #/ 1.8
          ...
          if (fs_id_link$fs_area[i] %in% areas_included) {
            ...
            if (Pf_eir > 0) {
              ...
              for (j in 1:N_reps) {
                ...
                if (camp_offset_from_peak) {
                  day_offset <- future_camps_month_sd * rnormvals[jj] * 365 / 12
                  day_offset <- day_offset + peak_season_dy
                  day_offset <- day_offset + month_offset_from_peak * 365 / 12
                  day_offset <- day_offset %% 365
                  camp_month_of_year <- round(12*day_offset/year)
                  peak_month <- round(12*peak_season_dy/year)
                  last_camp_month_index <- last_camp_month %% 12
                  proj_camp_override_month <- 12 * (last_camp %/% 12) + mass_int_mn + camp_month_of_year
                  proj_camp_override_int <- proj_camp_1 - last_camp
                  if (proj_camp_override_int < (mass_int_mn - 6)) {
                    proj_camp_override_month <- proj_camp_override_month + 12
                  } else if (proj_camp_override_int > (mass_int_mn + 6)) {
                    proj_camp_override_month <- proj_camp_override_month - 12
                  }
