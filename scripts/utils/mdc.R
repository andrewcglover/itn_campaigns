...
# Original function to estimate MDC timings
...
#-------------------------------------------------------------------------------
# Function to estimate uncertainty around MDC timings

estimate_mdc_uncertainty <- function(dataset, mdc_bounds_name, density_name) {
  for (i in 1:N_areas) {
    
    # Declare MDC vector
    MDC_tau <- rep(NA, N_CMC) # Initialise 
    ...
    for (j in 1:N_MDCs) {
      ...
      norm_sub <- sub_density / sum(sub_density)
      CMC_sub <- CMC_series[j0:j1]
      EX <- sum(norm_sub * CMC_sub) #equivalent to mean_mdc previously
      EX2 <- sum(norm_sub * CMC_sub^2)
      VarX <- EX2 - EX^2
      sdX <- sqrt(VarX)
      mdc_id <- which(CMC_series == mean_mdc)
