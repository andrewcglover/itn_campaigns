...
par_net_region_sequential_v2 <- function(param_list) {
  ...
  pfpr_max_bounds <- c(tail(prevalence_rendering_max_ages, 1),
                       prevalence_rendering_max_ages[4:5])
...
run_malsim_nets_sequential_v2 <- function(dataset,
                                          areas_included = NULL,
                                          N_reps = 100,
                                          N_cores = 0,
                                          areas_per_core = 1,
          ...
          if (fs_id_link$fs_area[i] %in% areas_included) {
            ...
            if (Pf_eir > 0) {
              ...
              for (j in 1:N_reps) {
                ...
                #   y <- long_sample_ids[(x-1) %% length(long_sample_ids) + 1]
                #   
                #   old_dat_res_filter <- old_dat_res %>%
                #     dplyr::filter(resistance == ref_res[kk])
                #   old_dat_res_sample[kk,] <- old_dat_res_filter[y,]
                #   
                #   new_dat_res_filter <- new_dat_res %>%
                #     dplyr::filter(resistance == ref_res[kk])
                #   new_dat_res_sample[kk,] <- new_dat_res_filter[y,]
                #   
                # }
                # 
                # # old_dat_res_sample <- old_dat_res %>%
                # #   dplyr:: filter(draw == sample_ids[jj])
                # # 
                # # new_dat_res_sample <- new_dat_res %>%
                # #   dplyr:: filter(draw == sample_ids[jj])
                # 
                # # Identify resistance id matches (same for old and new net types)
                # res_ids <- match(round_monthly_res,
                #                  round(old_dat_res_sample$resistance, 2))
                # 
                # # Create dn0 matrix
                # dn0_old <- old_dat_res_sample$dn0[res_ids]
                # dn0_vec <- new_dat_res_sample$dn0[res_ids]   # initially set for new net
                ...
                # 
                # # Create dn0 matrix
                # rn_old <- old_dat_res_sample$rn0[res_ids]
                # rn_vec <- new_dat_res_sample$rn0[res_ids]   # initially set for new net
                ...
                # gam_old <- 365 * old_dat_res_sample$gamman[res_ids] / log(2)
                # gam_vec <- 365 * new_dat_res_sample$gamman[res_ids] / log(2)
                # gam_vec[1:N_CMC_old_nets] <- gam_vec[1:N_CMC_old_nets]
  ...
  if (use_hipercow) {
    ...
    if (hiper_debug == TRUE) {
      par_id <- task_create_expr(par_net_region_sequential_v2(param_list[[1]]),
                                 resources = resources)
    } else {
      par_id <- task_create_expr(
        parallel::parLapply(NULL, param_list, par_net_region_sequential_v2),
    ...
    par_output <- parLapply(cl, param_list, par_net_region_sequential_v2)
    comb_output <- do.call(rbind.data.frame, par_output)
    output_df <- rbind(output_df, comb_output)
    stopCluster(cl)
  }
