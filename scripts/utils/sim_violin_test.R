...
sim_bar_plot <- function(sim_data,
  ...
  for (i in 1:length(fs_areas_included)) {
    ...
    single_adm_no_base %>%
    ggplot() +
      geom_violin(aes(x = net_strategy,
                      y = add_cases_averted_per_usd,
                      fill = net_strategy),
                  filter(single_adm_no_base, net_name == "pyrethroid-only"),
                  alpha = 0.5,
                  color = NA) +
      ...
      #                   ymax = var_hi,
      #                   color = net_strategy),
      #               filter(sim_sum_no_base, grepl("pyrethroid-only", net_strategy)),
      #               alpha = 1) +
      # geom_point(aes(x = net_strategy,
      #                y = var_mid,
      #                color = net_strategy),
      #            filter(sim_sum_no_base, grepl("pyrethroid-only", net_strategy)),
      #            alpha = 1) +
    #scale_fill_gradientn(colours = c("purple", "orange")) +
      ...
      #   #aesthetics = "colour"
      #) +
      labs(colour = "Pyrethroid-only") +
      #new_scale_color() +
      new_scale_fill() +
      geom_violin(aes(x = net_strategy,
                      y = add_cases_averted_per_usd,
                      fill = net_strategy),
                  filter(single_adm_no_base, net_name == "pyrethroid-pbo"),
                  alpha = 0.5,
                  color = NA) +
      # geom_errorbar(aes(x = net_strategy,
      #                   ymin = var_lo,
      #                   ymax = var_hi,
      #                   color = net_strategy),
      #               filter(sim_sum_no_base, grepl("pyrethroid-pbo", net_strategy)),
      #               alpha = 1) +
      # geom_point(aes(x = net_strategy,
      #                y = var_mid,
      #                color = net_strategy),
    ...
    scale_fill_viridis(
      ...
      # new_scale_color() +
      # new_scale_fill() +
      # geom_violin(data = single_adm_no_base %<>% filter(net_name == "pyrethroid-pyrrole"),
      #             aes(x = net_strategy,
      #                 y = add_cases_averted_per_usd,
      #                 fill = net_strategy),
      #             alpha = 0.5,
      #             position = "dodge",
      #             color = NA) +
      # geom_errorbar(data = sim_sum_no_base %<>% filter(grepl("pyrethroid-pyrrole", net_strategy)),
      #               aes(x = net_strategy,
      #                   ymin = var_lo,
      #                   ymax = var_hi,
      #                   color = net_strategy),
      #               alpha = 1) +
      # geom_point(data = sim_sum_no_base %<>% filter(grepl("pyrethroid-pyrrole", net_strategy)),
      #            aes(x = net_strategy,
      #                y = var_mid,
      #                color = net_strategy),
      #            alpha = 1) +
      ...
      #   #aesthetics = "colour"
      # ) +
      # labs(colour = "Pyrethroid-Pyrrole") +
      # ylab(yax_val) + 
      # xlab(NULL) +
