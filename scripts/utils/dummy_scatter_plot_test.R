cases_averted_scatter <- function(sim_sum,
  ...
  if (var_name == "uret") {sim_sum$var_val <- sim_sum$mean_uga}
  ...
  if (var_name == "uret") {plt <- plt + xlab("Mean use given access")}
...
quadrant_mean_retention <- function(sim_sum,
  ...
  if (xvar == "uret") {
    sim_sum$xmean <- sim_sum$mean_uga
    sim_sum$xLB <- sim_sum$LB_uga
    sim_sum$xUB <- sim_sum$UB_uga
    xround <- 1.5
    xbreaks <- seq(0,48,3)
  ...
  if (plot_quadrants) {
    plt <- plt +
      geom_hline(yintercept = vert_mid, colour = "black") +
      geom_vline(xintercept = horiz_mid, colour = "black")
  }
  if (xvar == "uret") {
    plt <- plt +
