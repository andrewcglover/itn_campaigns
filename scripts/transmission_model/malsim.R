...
future_subsite <- function(site = NULL, start_year = NULL) {
  fut_site <- list()
  fut_site$country <- site$country
  fut_site$version <- site$version
  fut_site$admin_level <- site$admin_level
  fut_site$sites <- site$sites
  fut_site$shape <- site$shape
  names(fut_site$shape) <- names(site$shape)
  fut_site$cases_deaths <- site$cases_deaths %>%
    dplyr::filter(year >= start_year)
  fut_site$prevalence <- site$prevalence %>%
    dplyr::filter(year >= start_year)
  fut_site$interventions <- site$interventions %>%
    dplyr::filter(year >= start_year)
  fut_site$population$population_total <- site$population$population_total %>%
    dplyr::filter(year >= start_year)
  fut_site$population$population_by_age <- site$population$population_by_age %>%
    dplyr::filter(year >= start_year)
  fut_site$demography <- site$demography %>%
    dplyr::filter(year >= start_year)
  fut_site$vectors$vector_species <- site$vectors$vector_species
  fut_site$vectors$pyrethroid_resistance <-
    site$vectors$pyrethroid_resistance %>%
    dplyr::filter(year >= start_year)
  fut_site$seasonality$seasonality_parameters <-
    site$seasonality$seasonality_parameters
  fut_site$seasonality$seasonality_parameters <-
    site$seasonality$monthly_rainfall %>%
    dplyr::filter(year >= start_year)
  fut_site$seasonality$fourier_prediction <- site$seasonality$fourier_prediction
  fut_site$blood_disorders <- site$blood_disorders
  fut_site$accessibility <- site$accessibility
  fut_site$eir <- site$eir
  return(fut_site)
}

par_net_region_sequential_v5 <- function(param_list) {
  ...
  clinical_incidence_rendering_min_ages <-
    site_pars$clinical_incidence_rendering_min_ages
  clinical_incidence_rendering_max_ages <-
    site_pars$clinical_incidence_rendering_max_ages
  severe_incidence_rendering_min_ages <-
    site_pars$severe_incidence_rendering_min_ages
  severe_incidence_rendering_max_ages <-
    site_pars$severe_incidence_rendering_max_ages
  EIR_fit <- site_pars$EIR_fit
  budget_pc <- site_pars$budget_pc
  past_sim <- site_pars$past_sim
  future_sim <- site_pars$future_sim
  past_end_year <- site_pars$past_end_year
  past_timesteps <- site_pars$past_timesteps
  past_path <- site_pars$past_path
  future_path <- site_pars$future_path
  summary_path <- site_pars$summary_path
  results_path <- site_pars$results_path
  urep_sf <- site_pars$urep_sf
  check_path <- site_pars$check_path
  load_previous_future_sim <- site_pars$load_previous_future_sim
  check_write <- site_pars$check_write
  ...
  P_ui <- P_u_samples[sid,] * urep_sf
  P0_ui <- P0_u_samples[sid,] * urep_sf
  D_ui <- D_u_samples[sid,] * urep_sf
  ...
  dist_used_future_mdc_only <- list(
    dist_used_future_mdc[[1]] - dist_used_no_future_mdc[[1]],
  ...
  # Future nets distributed through mass campaigns
  dist_access_future_mdc_only <- list(
    dist_access_future_mdc[[1]] - dist_access_no_future_mdc[[1]],
  ...
  if (net_costings | biennial_reduction) {
    ...
    for (t in new_start:new_end) {
      ...
      costed_P_r0[t] <-
        costed_P_npc_dist[t] * (1 - costed_P_r1[t]) + costed_P_r1[t]
      costed_P_a0[t] <- npc_beta * costed_P_r0[t] ^ npc_gamma
      costed_P_u0[t] <-
        predict(uga_approx, data.frame(aa = costed_P_a0[t]))
      
      # Costed usage distributed
      costed_u_dist[t] <-
        (costed_P_u0[t] - costed_P_u1[t]) / (1 - costed_P_u1[t]) 
  ...
  if (override_cost) {
    if (override_mdc_only) {
      avg_tail_nets_routine <-
        sum(tail(dist_used_no_future_mdc * tail_pop, n = 6 * 12)) / 6
      ...
      dist_used_future_mdc_only[new_net_range] <-
        dist_used_future_mdc_only[new_net_range] * override_cost_factor
      all_output_nets <- dist_used_no_future_mdc + dist_used_future_mdc_only
    } else {
      override_cost_factor <- override_cost_value / tail_net_cost
      all_output_nets[new_net_range] <-
        all_output_nets[new_net_range] * override_cost_factor
    }
  }
  
  # set bednets
  bednet_pars <- malariasimulation::set_bednets(
    site_pars,
  ...
  # set equilibrium
  bednet_pars <- malariasimulation::set_equilibrium(
    bednet_pars,
    init_EIR = EIR_fit
  )
  
  # get correlations
  sim_corr <- malariasimulation::get_correlation_parameters(bednet_pars)
  
  # past or future sim
  if (past_sim) {
    past_output <- malariasimulation::run_resumable_simulation(
      timesteps = past_timesteps,
      parameters = bednet_pars,
      correlations = sim_corr
    )
    saveRDS(
      past_output,
      file = file.path(
    ...
    #dir.create(check_path, recursive = TRUE)
    if (check_write) {write("", file = file.path(check_path, "checkm5.txt"))}
    if (load_previous_future_sim) {
      future_output <- readRDS(
        file = file.path(
          future_path,
          paste0("sim", sid, ".rds")
        )
      ...
      future_output <- malariasimulation::run_resumable_simulation(
        timesteps = bednet_pars$timesteps,
        parameters = bednet_pars,
        initial_state = past_output$state,
        restore_random_state = TRUE
      )
    }
    if (check_write) {write("", file = file.path(check_path, "checkm4.txt"))}
    saveRDS(
      future_output,
      file = file.path(future_path, paste0("sim", sid, ".rds"))
    )
    if (check_write) {write("", file = file.path(check_path, "checkm3.txt"))}
    # Check if columns have been dropped in future simulation
    # Note, this can happen due to stochastic elimination, in which case missing
    # cols with zeros are reinserted.
    if (
      (length(names(past_output$data)) != length(names(future_output$data))) |
      (any(names(past_output$data) != names(future_output$data)))
    ) {
      # Identify missing columns
      missing_cols <- setdiff(
        names(past_output$data), names(future_output$data)
      )
      # Add each missing column with 0s
      for (col in missing_cols) {
        future_output$data[[col]] <- 0
      }
      # Reorder columns to match past_output$data
      future_output$data <- future_output$data[, names(past_output$data)]
    }
    # Note that rbind.data.frame will align col order of future output so that
    # it matches that of past output
    output <- rbind.data.frame(past_output$data, future_output$data)
    #checkm2 <- -2
    #saveRDS(checkm2, file = file.path(check_path, "checkm2.rds"))
  } else {
  ...
  if (!(past_sim)) {
    ...
    if (check_write) {write("", file = file.path(check_path, "checkm1.txt"))}
    ...
    annual_output <- as.data.frame(
      matrix(rep(0, projection_window_yr * (21 + total_age_cols)),
             nrow = projection_window_yr)
    )
    all_summary_names <- c(
    ...
    if (check_write) {write("", file = file.path(check_path, "check0.txt"))}
    ...
    annual_output$biennial_reduction <- rep(
      biennial_reduction, projection_window_yr
    )
    ...
    annual_output$routine_baseline <- rep(
      routine_baseline, projection_window_yr
    )
    ...
    if (check_write) {write("", file = file.path(check_path, "check1.txt"))}
    
    if (no_future_nets | routine_baseline) {
      #\data_results\urep_06_20250604112237\future\BF\Pyrethroid-Pyrrole_2_year_interval\fs_id_1
      ref_2yr <- readRDS(
        file = file.path(
          paste(
            results_path,
            ...
            paste0("fs_id_", fs_area_id),
            sep = "/"
          ),
          paste0("sim", sid, ".rds")
        )
      )
      if (check_write) {write("", file = file.path(check_path, "check1p1.txt"))}
      ref_2yr_CMC_start <- ref_2yr$CMC_start
      if (check_write) {write("", file = file.path(check_path, "check1p2.txt"))}
      ref_3yr <- readRDS(
        file = file.path(
          paste(
            results_path,
      ...
      if (check_write) {write("", file = file.path(check_path, "check1p3.txt"))}
      ref_3yr_CMC_start <- ref_3yr$CMC_start
      if (check_write) {write("", file = file.path(check_path, "check1p4.txt"))}
      no_camp_CMC_start <- round(
        rowMeans(cbind(ref_2yr_CMC_start, ref_3yr_CMC_start))
      )
      if (check_write) {write("", file = file.path(check_path, "check1p5.txt"))}
      no_camp_CMC_start <- seq(
        from = no_camp_CMC_start[1],
        by = (no_camp_CMC_start[2] - no_camp_CMC_start[1]),
        length.out = projection_window_yr
      )
      if (check_write) {write("", file = file.path(check_path, "check2.txt"))}
    }
    for (t in 1:length(output_net_times)) {
      ...
      t11 <- t + 11
      if (t11 > length(output_net_times)) {t11 <- length(output_net_times)}
      if (no_future_nets | routine_baseline) {
        #if ((CMC_Jan2000 + t - 1) %in% no_camp_CMC_start) {
        if (
          (t >= (no_camp_CMC_start[1] - CMC_Jan2000)) &
          (((t - (no_camp_CMC_start[1] - CMC_Jan2000 + 1)) %% 12 ) == 0)
        ) {
          #if (((t - new_net_start_mn) %% 12 ) == 0) {
          if (check_write) {
            write("", file = file.path(check_path, paste0("check3_",t,".txt")))
          }
          yr <- yr + 1
          annual_output[yr,"year_id"] <- yr
          annual_output[yr, "CMC_start"] <- CMC_Jan2000 + t - 1
          annual_output[yr, "CMC_end"] <- CMC_Jan2000 + t11 - 1
          ...
          if (check_write) {
            write("", file = file.path(check_path, paste0("check4_",t,".txt")))
          }
        ...
        if (t >= first_new_net_MDC) {
          if ((m_proj[t - first_new_net_MDC + 1] %% 12) == 0) {
            ...
            tot_npc_this_yr <- sum(P_npc_dist[t:(t11)])
      ...
      #   # }
      #   # if (no_future_nets | routine_baseline) {
      #   #   if ((m_proj[t - N_early] - 30) >= 0) {
      #   #     if (((m_proj[t - N_early] - 30) %% 12 ) == 0) {
      #   #       #if (((t - new_net_start_mn) %% 12 ) == 0) {
      #   #       yr <- yr + 1
      #   #       annual_output[yr,"year_id"] <- yr
      #   #       annual_output[yr, "CMC_start"] <- CMC_Jan2000 + t - 1
      ...
      #   if (
      #     (
      #       !(no_future_nets | routine_baseline) &
      #       ((m_proj[t - first_new_net_MDC + 1] %% 12) == 0)
      #     ) | (
      #       (no_future_nets | routine_baseline) &
      #       ((CMC_Jan2000 + t - 1) %in% no_camp_CMC_start)
      #     )
      #   ) {
      #     yr <- yr + 1
      #     annual_output[yr,"year_id"] <- yr
      ...
      #   }
      #   #}
      if (yr > 0 & yr <= dim(annual_output)[1]) {
        if (check_write) {
          write("", file = file.path(check_path, paste0("check5_",t,".txt")))
        }
        ...
        for (tt in t0:t1) {
          ...
          if (check_write) {
            write("", file = file.path(check_path, paste0("check6_",t,".txt")))
          }
        }
    ...
    annual_output <- annual_output[1:6,]
    
    if (check_write) {write("", file = file.path(check_path, "check7.txt"))}
    
    saveRDS(
      annual_output,
      file = file.path(summary_path, paste0("sim", sid, ".rds"))
    )
...
#   hiper_debug = FALSE,
#   CMC_last = 1500,
#   projection_window_yr = 10,
#   budget_pc = 100)
# 
# list2env(debug_list, envir = .GlobalEnv)


run_malsim_nets_sequential_v5 <- function(
    #dataset,
  areas_included = NULL,
  N_reps = 100,
  N_cores = 0,
  ...
  past_end_year = 2024,
  future_sim = FALSE,
  results_path = "./data_results/06_testing",
  past_path = NULL,
  future_path = NULL,
  summary_path = NULL,
  ...
  urep_sd = 5,
  null_return = TRUE,
  load_previous_future_sim = FALSE,
  check_write = FALSE) {
  ...
  # # Projection window
  # if (past_sim) {projection_window_yr <- 0}
  ...
  past_timesteps <- (past_end_year - 2000 + 1) * 365 # site files start at 2000
      ...
      for (k in 1:N_int_vals) {
        ...
        for (i in 1:N_fs_areas) {
          ...
          if (fs_id_link$fs_area[i] %in% areas_included) {
            ...
            # Isolate single admin site and switch urban/rural if no site found
            adm_site_index <- which(
              ctry_site$sites$name_1 == fs_id_link$fs_name_1[i] &
                ctry_site$sites$urban_rural == fs_id_link$urbanicity[i]
            )
            if (identical(adm_site_index, integer(0))) {
              if (fs_id_link$urbanicity[i] == "urban") {adm_urb <- "rural"}
              if (fs_id_link$urbanicity[i] == "rural") {adm_urb <- "urban"}
            } else {
              adm_urb <- fs_id_link$urbanicity[i]
            }
            ...
            # Create folders
            if (fs_id_link$fs_area_id[i] == i) {
              if (past_sim | future_sim) {
                past_path <- paste(
                  results_path,
                  "past",
                  fs_id_link$ISO2[i],
                  paste0(mass_int_yr[k], "_yr_end"),
                  paste0("fs_id_", fs_id_link$fs_area_id[i]),
                  sep = "/"
                )
                dir.create(past_path, recursive = TRUE)
                # file = file.path(
                #   paste(past_path, sid, sep = "/"),
                #   paste0("state", sid, ".rds")
                # )
                # past_path <- paste(
                #   results_path, "past", ISO2, paste0("fs_id_", fs_area_id),
                #   sep = "/"
                #)
                # for (repx in 1:N_reps) {
                #   dir.create(paste(past_path, repx, sep = "/"), recursive = TRUE)
                # }
            ...
            # Pf EIR
            if (under_reporting) {
              adm_site$eir$eir[1] <- fs_id_link$EIR_urep_fit[i]
            } else {
            ...
            if (Pf_eir > 0) {
              ...
              site_pars$past_sim <- past_sim
              site_pars$future_sim <- future_sim
              site_pars$past_end_year <- past_end_year
              site_pars$past_timesteps <- past_timesteps
              
              site_pars$past_path <- past_path
              site_pars$future_path <- future_path
              site_pars$summary_path <- summary_path
              site_pars$results_path <- results_path
              
              site_pars$check_write <- check_write
              site_pars$load_previous_future_sim <- load_previous_future_sim
              ...
              for (j in 1:N_reps) {
                ...
                for (kk in 1:N_CMC_sim) {
                  #for (kk in 1:length(round_monthly_res)) {
                ...
                if (under_reporting) {
                  urep_pc <- urep_mean + rnormvals[jj] * urep_sd / 1.96
                  if (urep_pc < 0) {urep <- 0}
                  urep_sf <- 1 - urep_pc / 100
                } else {
                  urep_sf <- 1
                }
                site_pars$urep_sf <- urep_sf
                ...
                dir.create(check_path, recursive = TRUE)
                site_pars$check_path <- check_path
  ...
  if (use_hipercow) {
    ...
    if (hiper_debug == TRUE) {
      par_id <- task_create_expr(par_net_region_sequential_v5(param_list[[1]]),
                                 resources = resources)
      # par_id <- task_create_expr(par_net_region_sequential_v5(param_list[[2]]),
      #                            resources = resources)
    } else if (N_cores == 1) {
      # par_id <- task_create_expr(
      #   lapply(param_list, par_net_region_sequential_v5)
      # )
      par_id <- lapply(param_list, function(p) {
        task_create_expr(par_net_region_sequential_v5(p))
      })
    } else {
      par_id <- task_create_expr(
        parallel::parLapply(NULL, param_list, par_net_region_sequential_v5),
    ...
    par_output <- parLapply(cl, param_list, par_net_region_sequential_v5)
    comb_output <- do.call(rbind.data.frame, par_output)
    output_df <- rbind(output_df, comb_output)
    stopCluster(cl)
  }
...
# }
# 
# par_net_region_sequential_v6 <- function(param_list) {
#   
#   #library(magrittr)
#   
#   #N_reps <- length(param_list)
...
#   
#   # set equilibrium
#   bednet_pars <- malariasimulation::set_equilibrium(bednet_pars,
#                                                     init_EIR = EIR_fit)
#   
#   # get correlations
#   sim_corr <- malariasimulation::get_correlation_parameters(bednet_pars)
...
#     #dir.create(check_path, recursive = TRUE)
#     #checkm5 <- -5
#     #saveRDS(checkm5, file = file.path(check_path, "checkm5.rds"))
#     future_output <- malariasimulation::run_resumable_simulation(
#       timesteps = bednet_pars$timesteps,
#       parameters = bednet_pars,
#       initial_state = past_output$state,
...
#     )
#     #checkm4 <- -4
#     #saveRDS(checkm4, file = file.path(check_path, "checkm4.rds"))
#     saveRDS(
#       future_output,
#       file = file.path(future_path, paste0("sim", sid, ".rds"))
#     )
#     #checkm3 <- -3
#     #saveRDS(checkm3, file = file.path(check_path, "checkm3.rds"))
#     output <- rbind.data.frame(past_output$data, future_output$data)
#     #checkm2 <- -2
#     #saveRDS(checkm2, file = file.path(check_path, "checkm2.rds"))
#   } else {
...
#     
#     #checkm1 <- -1
#     #saveRDS(checkm1, file = file.path(check_path, "checkm1.rds"))
#     
#     pfpr_min_bounds <- prevalence_rendering_min_ages[1:3]
#     pfpr_max_bounds <- c(tail(prevalence_rendering_max_ages, 1),
#                          prevalence_rendering_max_ages[3:4])
...
#     
#     #check0 <- 0
#     #saveRDS(check0, file = file.path(check_path, "check0.rds"))
#     
#     names(annual_output) <- all_summary_names
#     annual_output$ISO2 <- rep(ISO2, projection_window_yr)
#     annual_output$fs_name_1 <- rep(fs_name_1, projection_window_yr)
...
#     
#     #check1 <- 1
#     #saveRDS(check1, file = file.path(check_path, "check1.rds"))
#     
#     if (no_future_nets | routine_baseline) {
#       #\data_results\urep_06_20250604112237\future\BF\Pyrethroid-Pyrrole_2_year_interval\fs_id_1
#       ref_2yr <- readRDS(
...
#       )
#       #check2 <- 2
#       #saveRDS(check2, file = file.path(check_path, "check2.rds"))
#     }
#     for (t in 1:length(output_net_times)) {
#       t0 <- output_net_times[t]
#       if (t == length(output_net_times)) {
...
#       if (no_future_nets | routine_baseline) {
#         #if ((CMC_Jan2000 + t - 1) %in% no_camp_CMC_start) {
#         if (((t - (no_camp_CMC_start[1] - CMC_Jan2000 + 1)) %% 12 ) == 0) {
#         #if (((t - new_net_start_mn) %% 12 ) == 0) {
#           #check3 <- 3
#           #saveRDS(check3, file = file.path(check_path, paste0("check3_",t,".rds")))
#           yr <- yr + 1
#           annual_output[yr,"year_id"] <- yr
#           annual_output[yr, "CMC_start"] <- CMC_Jan2000 + t - 1
#           annual_output[yr, "CMC_end"] <- CMC_Jan2000 + t11 - 1
...
#           )
#           #check4 <- 4
#           #saveRDS(check4, file = file.path(check_path, paste0("check4_",t,".rds")))
#           
#         }
#       } else {
#         if (t >= first_new_net_MDC) {
...
#       if (yr > 0 & yr <= dim(annual_output)[1]) {
#         #check5 <- 5
#         #saveRDS(check5, file = file.path(check_path, paste0("check5_",t,".rds")))
#         for (a in 1:length(n_age_cols)) {
#           annual_output[yr, n_age_cols[a]] <- (
#             annual_output[yr, n_age_cols[a]] + sum(
#               output[
...
#           )
#           #check6 <- 6
#           #saveRDS(check6, file = file.path(check_path, paste0("check6_",t,".rds")))
#         }
#         
#       }
#       
...
#     
#     #check7 <- 7
#     #saveRDS(check7, file = file.path(check_path, "check7.rds"))
#     
#     
#     saveRDS(
#       annual_output,
...
# 
# 
# run_malsim_nets_sequential_v6 <- function(
#     #dataset,
#   areas_included = NULL,
#   N_reps = 100,
#   N_cores = 0,
...
#               site_pars$results_path <- results_path
#               
#               # Pre-split resistance data once
#               old_res_map <- split(old_dat_res, old_dat_res$resistance)
#               new_res_map <- split(new_dat_res, new_dat_res$resistance)
#               
#               # Precompute sample index matrix
#               jj_vec <- seq_len(N_reps) + rep_offset
#               sample_matrix_idx <- matrix(NA_integer_, nrow = N_reps, ncol = N_CMC_sim)
#               for (j in seq_len(N_reps)) {
#                 jj <- jj_vec[j]
#                 for (kk in seq_len(N_CMC_sim)) {
#                   x <- (jj - 1) * N_CMC_sim + kk
#                   sample_matrix_idx[j, kk] <- long_sample_ids[(x - 1) %% length(long_sample_ids) + 1]
#                 }
#               }
#               
#               for (j in seq_len(N_reps)) {
...
#                 
#                 this_pars <- site_pars
#                 this_pars$sample_index <- jj
#                 this_pars$month_offset <- long_month_offset[jj]
#                 this_pars$mean_retu <- ret_u_samples[jj]
#                 this_pars$mean_reta <- ret_a_samples[jj]
#                 this_pars$npc_beta <- npc_beta_samples[jj]
#                 this_pars$npc_gamma <- npc_gamma_samples[jj]
#                 
#                 # Build resistance sample in vectorized manner
#                 res_rows <- vector("list", N_CMC_sim)
#                 #last_res_val <- NULL
#                 last_res_val <- NA_character_
#                 
#                 for (kk in seq_len(N_CMC_sim)) {
#                   y <- sample_matrix_idx[j, kk]
#                   res_val <- as.character(round_monthly_res[kk])
#                   
#                   if (res_val != last_res_val) {
#                     last_res_val <- res_val
#                     res_kk <- if (kk < N_CMC_old_nets) old_res_map[[res_val]] else new_res_map[[res_val]]
#                   }
#                   
#                   sample_kk <- res_kk[res_kk$draw == y, , drop = FALSE]
#                   if (nrow(sample_kk) < 1) {
#                     warning(sprintf("Warning: %s resistance match fail", res_val))
#                   }
#                   res_rows[[kk]] <- sample_kk
#                 }
#                 
#                 comb_dat_res_sample <- do.call(rbind, res_rows)
#                 
#                 this_pars$dn0_mat <- matrix(rep(comb_dat_res_sample$dn0, N_species),
#                                             nrow = N_CMC_sim, ncol = N_species)
#                 this_pars$rn_mat  <- matrix(rep(comb_dat_res_sample$rn0, N_species),
#                                             nrow = N_CMC_sim, ncol = N_species)
#                 this_pars$rnm_mat <- matrix(0.24, nrow = N_CMC_sim, ncol = N_species)
#                 this_pars$gam_vec <- 365 * comb_dat_res_sample$gamman / log(2)
#                 
#                 # Under-reporting scale factor
#                 if (under_reporting) {
#                   urep_pc <- urep_mean + rnormvals[jj] * urep_sd / 1.96
#                   this_pars$urep_sf <- max(0, 1 - urep_pc / 100)
#                 } else {
#                   this_pars$urep_sf <- 1
#                 }
#                 
#                 check_path <- file.path(results_path, "check",
#                                         fs_id_link$ISO2[i],
#                                         gsub(" ", "_", net_strategy),
#                                         paste0("fs_id_", fs_id_link$fs_area_id[i]),
#                                         paste0("sim", jj))
#                 
#                 if (!dir.exists(check_path)) dir.create(check_path, recursive = TRUE)
#                 this_pars$check_path <- check_path
#                 
#                 param_list[[length(param_list) + 1]] <- this_pars
#               }
#               
#               
#             }
...
#   if (use_hipercow) {
#     resources <- hipercow_resources(cores = N_cores)
#     if (hiper_debug == TRUE | length(param_list) == 1) {
#       par_id <- task_create_expr(par_net_region_sequential_v6(param_list[[1]]),
#                                  resources = resources)
#     } else {
#       par_id <- task_create_expr(
#         parallel::parLapply(NULL, param_list, par_net_region_sequential_v6),
#         parallel = hipercow_parallel("parallel"),
#         resources = resources)
#     }
#     
...
#       "population_usage_t"))
#     #par_output <- lapply(param_list, par_net_region_sequential3)
#     par_output <- parLapply(cl, param_list, par_net_region_sequential_v6)
#     comb_output <- do.call(rbind.data.frame, par_output)
#     output_df <- rbind(output_df, comb_output)
#     stopCluster(cl)
#   }
