  ...
  for (i in 1:N_fs_areas) {
    ...
    if (fs_id_link$fs_area[i] %in% areas_included) {
      ...
      P_u_mid <- P_u[, area_time_ids] %>%
        as.matrix %>% unname %>% colMeans %>% multiply_by(usage_scale_factor)
      ...
      # Prevalence
      fs_area_id_selected <- fs_area_id
      adm_dataset <- dataset %>% dplyr::filter(fs_area_id == fs_area_id_selected)
