...
par_EIR_fit <- function(param_list) {
  ...
  bednet_pars$human_population <- 1e6
  bednet_pars <- set_equilibrium(bednet_pars, init_EIR = EIR_out)
  raw <- run_simulation(bednet_pars$timesteps, parameters = bednet_pars)
...
run_eir_calibration <- function(
  ...
  for (i in 1:N_fs_areas) {
    ...
    if (fs_id_link$fs_area[i] %in% areas_included) {
      ...
      # Prevalence
      adm_dataset <- dataset %>% filter(area_id == area_id)
