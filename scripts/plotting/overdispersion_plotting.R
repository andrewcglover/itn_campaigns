...
# Functions for plotting the equity of ITN use and access within subnational
# regions, using the fitted overdispersion parameters (alpha0) that describe
# subregional heterogeneity in the probability of use and access
# Generates figure 5 and its figure supplements in Glover et al. (2025)
# Corresponds to appendix 4 of the paper, equation for the Beta distribution of
# individual-level use and access
#
# Consumes the globals overdisp_u and overdisp_a, which are assigned in
# ./use_access_fitting/usage_access_fitting.R; run after 04_use_access_fitting.R
# Not sourced by 00_initialisation.R
#
# The three figures reported in the paper are produced by
# plot_mean_overdispersion() as follows:
#
#   manual_cols = TRUE  -> <country>_subset_yr1_3_overdisp_50CrI*.pdf  (figure 5)
#   plot_usage  = TRUE  -> <country>_yr1_3_overdisp_50CrI_usage*.pdf
#   plot_access = TRUE  -> <country>_yr1_3_overdisp_50CrI_access*.pdf
#
# with country = "SN" for those reported. Its input, overdisp_df_rep, is
# generated by repeated_cumulative_overdispersion() below.
#
# Note this file was recovered from a local snapshot predating the 2025
# restructuring of the scripts folder, during which it was not carried across.
# The globals it consumes are unchanged, but it has not been re-run since.
