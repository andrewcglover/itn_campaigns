dat_res_pyr_ll <- readRDS("./data/pyrethroid_uncertainty 2") %>%
...
dat_res_pbo_ll <- readRDS(
  "./data/pbo_uncertainty_using_pyrethroid_dn0_for_mn_durability 2"
  ) %>%
...
dat_res_pp_ll <- readRDS(
  "./data/pyrrole_uncertainty_using_pyrethroid_dn0_for_mn_durability 2"
  ) %>%
  dplyr::rename(rn0 = rn_pbo, gamman = mn_dur, resistance = resistance.x) %>%
  dplyr::mutate(bioassay_mortality = 1 - resistance) %>%
  dplyr::select(dn0, rn0, gamman, resistance, bioassay_mortality) %>%
...
dat_res_pyr <- readRDS("./data/pyrethroid_binomial_uncertainty 1") %>%
  dplyr::rename(rn0 = rn_pyr, gamman = mean_duration) %>% 
  dplyr::mutate(bioassay_mortality = 1 - resistance)  %>%
  dplyr::select(dn0, rn0, gamman, resistance, bioassay_mortality) %>%
  cbind.data.frame("draw" = rep(1:1000, each = 101)) #%>%
...
dat_res_pbo <- readRDS(
  "./data/pbo_binomial_uncertainty_using_pyrethroid_dn0_for_mn_durability 1"
  ) %>%
  dplyr::rename(rn0 = rn_pbo, gamman = mn_dur, resistance = resistance.x) %>%
  dplyr::mutate(bioassay_mortality = 1 - resistance) %>%
  dplyr::select(dn0, rn0, gamman, resistance, bioassay_mortality) %>%
...
dat_res_pp <- readRDS(
  "./data/pyrrole_binomial_uncertainty_using_pyrethroid_dn0_for_mn_durability 1"
  ) %>%
  dplyr::rename(rn0 = rn_pbo, gamman = mn_dur, resistance = resistance.x) %>%
  dplyr::mutate(bioassay_mortality = 1 - resistance) %>%
  dplyr::select(dn0, rn0, gamman, resistance, bioassay_mortality) %>%
